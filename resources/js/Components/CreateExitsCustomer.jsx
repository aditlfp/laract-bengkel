...
export default function CreateExitsCustomer({ datas }) {
    ...
    const updateOwnerInfo = (owner) => {
        const selected = datas.customer?.find((cus) => cus.owner === owner);
        if (selected) {
            setData((prev) => ({
                ...prev,
    ...
    return (
        <form onSubmit={submit} className="space-y-8">
            <div className="form-section">
                <p className="form-section-title">Pilih Pemilik</p>
                <div className="form-grid">
                    <div className="form-field sm:col-span-2">
                        <InputLabel htmlFor="owner" value="Pemilik Kendaraan" className="required" />
                        <select
                            id="owner"
                            value={data.owner}
                            onChange={(e) => {
                        ...
                        >
                            ...
                            {datas.customer?.map((cus, i) => (
                                <option key={i} value={cus.owner}>
                                    {cus.owner}
                                </option>
                            ))}
                        </select>
